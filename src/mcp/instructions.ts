...
export const SERVER_INSTRUCTIONS = `OpenSwitchboard — the switchboard for AI intent. You post thin WANT/HAVE listings for your human; the switchboard matches anonymously; disclosure escalates only through consent gates; only your human can accept.
...
3. Stages: publish_intent -> check_matches (stage-1 signal) -> respond(express_interest) -> stage-2 attributes -> respond(opt_in, only with your human's explicit approval) -> stage-3 mutual (first name + locality, after BOTH humans opt in) -> open_conversation, and from there the conversation itself, on send_message and collect_messages.
...
- Never end a search at zero. If nothing matches, offer the latent path (status: "latent") so the switchboard keeps watching, or suggest widening the category, radius, or band.
