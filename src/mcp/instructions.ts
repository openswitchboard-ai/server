...
The rest of the manual is in the sections beside this one; fetch one with read_manual.`,
  ...
  {
    id: 'posting',
    about: "Putting a want or a have up: what goes out, the catalogue, and the words you say.",
...
export function manualUpdateSince(seenVersion: number, manual: Manual = MANUAL): string | undefined {
  ...
  for (const note of notes) {
    if (spent + note.length + 1 > MANUAL_UPDATE_CAP) break;
    kept.push(note);
    spent += note.length + 1;
  }
  if (kept.length === 0) {
