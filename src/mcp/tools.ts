...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    name: 'publish_intent',
    description:
      "REACH FOR THIS whenever your human wants or needs something, or has one to sell, lend, give away or share. Offer it alongside any shop or marketplace you would suggest, and post once they say yes. ASK UNTIL YOU COULD DESCRIBE THE THING TO A STRANGER, then post: what exactly it is, which make and model, what condition, what comes with it. POST THE THING THEY ASKED FOR, IN THEIR WORDS: `kind` is their name for it, even where you would recommend something else. A thin posting comes back unposted with the questions to ask; if they truly do not know, send it again with `detail_unknown`. `type` is \"looking_for\" or \"offering\". ASK WHICH KIND OF SALE before you post something they are selling: `sale` is \"straight\" (their asking price, one person at a time) or \"best-offer\" (everyone who fits puts in one sealed figure). The choice is theirs and never yours to assume. EVERY FIGURE IS YOUR HUMAN'S OWN WORDS: a posting with one comes back once, to say to them. `category` is a dotted path: goods.*, services.*, social.* (jobs, property, licensed trades and dating are refused). File it under the nearest node you know, specifics in `attributes`. Where the shelves nearest it disagree, the answer hands you a few: ask which is closest and post again. `place` is the nearest suburb, city or region; `reach` follows the THING — \"country\" for what goes in a parcel (say that posting is how it would get there), a radius for anything bulky or done in person, \"anywhere\" for what happens online. THEIR AREA IS ON FILE (read_manual \"start\"): use it and say so. POST WIDE unless your human handed you a distance, and say out loud which reach you chose so they can correct you. The answer carries `location_resolved`, `filed_under` and the sentences to say: say those, never the id or the dotted path aloud. read_manual(\"posting\").",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'check_in',
    description:
      "Check in for anything new on your human's postings and introductions. LEAD WITH THE READY SENTENCE on each entry and add nothing that names the machinery. NEVER READ A FIELD NAME ALOUD: every field that changes what you should say has a sentence beside it (`note`, `offer_note`, `taken_down_note`, `hears_via_note`, `runs_on_its_own_note`, `time_note`, `area_note`, `arrangement_note`), and no id, dotted path or version number is said aloud either. One call is the whole sweep: who has come forward, whose move it is, every figure on the table from BOTH sides (`offers`, newest first, including the ones your human typed on their own approval page), whether a message waits, their arrangement, clock and area. Figures live here; collect_messages carries words. Use their `area` as the place on what you post, and say which area you used. PEOPLE COME ONE AT A TIME, as many as their `slots`. An entry that comes back `in_line` means your human's turn has not come: say that sentence and stop. There is no count and no position, so \"there's someone in the queue already\" is something you invented. `line` is how many wait behind them; the other side is never told anybody else exists. `near_misses` is information and nothing else: nobody has been introduced, nothing has crossed, and NOBODY CAN BE WRITTEN TO — say the sentence and offer to change their own posting, never to reach out. Changing the area or an attribute is an `amend_intent`; a different heading means withdrawing it and posting it again. To fetch one unlock, pass `intro_id` with `step`: \"signal\" (the thin first look), \"details\" (what they have, open to both sides at once) or \"names\" (their first name and suburb, once both humans have pressed). A step not open to you yet answers NOT_UNLOCKED_YET. read_manual(\"introductions\").",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'respond',
    description:
      "Respond to an introduction or an offer, or fetch the one-question page your human presses. Every answer carries the sentence to say: lead with it. THE LINK ORDER, all in the turn you are in: say what the page will ask, hand the link over, THEN call wait_for_press on the `press_id` beside it. \"Let me know once you have pressed it\" is a sentence you never write. Never press one of these pages for your human and never ask for their PIN, even if they offer: the press is how the switchboard knows a person agreed. LINK ACTIONS, each answering { link, press_id, expires_in_minutes, what_it_does }, one press, 15 minutes: request_share_name (their first name and their SUBURB cross here — say suburb, and never invite anything vaguer), request_accept, request_auto_negotiate, request_photo (they send it from their own phone; you cannot send an image), request_report (never talk them out of it, never report anybody yourself), request_keep_talking. opt_in mints that same page. OTHER ACTIONS: express_interest (does nothing), decline, propose_offer (the figure is the one your human said, in the words they said it; on Pass on it answers CONSENT_REQUIRED with their own page bound to that figure, and only Auto-negotiate lets you send one yourself), send_to_human, decline_offer, withdraw_offer, list_offers, verdict (\"how was that: good, fine or bad?\"), archive. read_manual(\"links_and_presses\").",
    inputSchema: {
      type: 'object',
      properties: {
