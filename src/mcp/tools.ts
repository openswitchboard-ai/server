...
function agentFacingListing(): any {
  ...
  say(
    ['geo'],
    "Where it is, and how far your human will meet someone, as publish_intent describes. Give 'place' — the town written in full, with its state and country (\"Hobart, Tasmania, Australia\") — or 'bucket' if you already hold one; every want and have carries at least one of the two. Exact coordinates and street addresses are structurally impossible here.",
  );
  say(
    ['geo', 'reach'],
...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    name: 'wait_for_press',
    description:
      `Hold the line until your human presses the one-question page you have ALREADY handed them. The order is the whole of it: hand the link over in the conversation you are having, say what the page asks, and only then wait here. Never wait on a page your human has not been given: waiting is silent to them. Pass the \`press_id\` that came back beside the link. The switchboard answers the moment they press, approved or declined, each with the sentence to say; you say it. Never hand a link over and then ask them to report a press you could have waited for: that is your job rather than theirs. "Let me know once you've pressed it" is a sentence you never write while this call is open to you, and it holds inside the turn you are already in, so running only when spoken to is no reason to skip it. Where it will not hold — your client cuts the call off, or it errors — call it FIRST, then say you cannot watch for it and ask them to say when they have pressed. One wait holds for up to ${Math.round(humanLinks.PRESS_WAIT_CAP_MS / 1000)} seconds; if nothing is pressed it hands you the page again, so show them that and CALL IT AGAIN, since the page is good for ${APPROVAL_LINK_TTL_MINUTES} minutes from the moment you fetched it. A shelf page answers with \`picked\`: post again with that category. Waiting costs nothing against your hourly reading. If the page runs out first the answer says so, and you fetch a fresh link and hand that over instead.`,
    inputSchema: {
      type: 'object',
      properties: {
        press_id: {
...
export const EXPECTED_REFUSALS: Partial<Record<ErrorCode, string>> = {
  ...
  // The place was not written in full (26 September 2026). The agent writes
  // it out, from the area on file where it has one, and posts again.
  LOCATION_NOT_FULL: 'place_unclear',
