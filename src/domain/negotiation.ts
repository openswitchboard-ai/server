...
export function checkAgainstMandate(
  ...
  if (input.ccy.toUpperCase() !== m.ccy) {
    return { ok: false, reason: `Your human's numbers here are in ${m.ccy}, and this offer is in ${input.ccy.toUpperCase()}.` };
  }
  ...
  if (input.isOpening && m.open !== undefined && amount !== m.open) {
    return { ok: false, reason: `Your human said to open at ${m.open} ${m.ccy}.` };
  }
