...
describe('counter pages: copy-cull render suite', () => {
  const allPages = (): { name: string; html: string }[] => [
    ...
    { name: 'pin-change', html: cpages.pinSetPage(undefined, { hasPin: true }) },
    // The step that used to demand a PIN: a passkey or a PIN, the person's
    // pick, and either one finishes registration on its own.
    { name: 'credential-choice', html: cpages.credentialChoicePage() },
    { name: 'passkey-offer', html: cpages.passkeyOfferPage() },
    { name: 'passkey-offer-second', html: cpages.passkeyOfferPage({ hasPasskey: true }) },
    { name: 'security-pin-only', html: cpages.securityPage({ hasPin: true, passkeyCount: 0 }) },
    {
      name: 'security-passkey-only',
      html: cpages.securityPage({ hasPin: false, passkeyCount: 2 }),
    },
    {
      name: 'confirm-its-you-passkey',
      html: cpages.confirmItsYouPage(
        { hasPin: false, hasPasskey: true, elevated: false },
        '/pin',
      ),
    },
    {
